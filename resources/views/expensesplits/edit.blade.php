...
<form class="form-horizontal" action="/expensesplits/{{$expense->id}}" method="POST" autocomplete="off">
	{{ csrf_field() }}
	{{ method_field('PATCH') }}
...
<div class="row">
	<div id="sections">
		@if (old() == true)
			...
			@foreach($expense_splits as $i => $expense_split)
			  	<div class="section">
					<div class="col-md-12">
				...
				<input type="hidden" name="expense_split_id[]" value="{{ $expense_split->id }}">
			...
			@endforeach
			<?php $count = count(old('amount')); ?>
			@for($i = $expense_splits->count(); $i < $count; ++$i)
			  	<div class="section">
					<div class="col-md-12">
						<div class="panel panel-default">
							<div class="panel-heading">
							...
							<div class="panel-body">
				    			<fieldset>
				       				@include('expensesplits._form')   								
									<input type="hidden" name="expense_split_id[]" value="">
				    			</fieldset>
			 				</div>
			 				<div class="panel-footer">
								<a href="" class='btn btn-primary addsection'>Add Split</a>
		...
		@else
			<?php $count = $expense_splits->count(); ?>
			@foreach($expense_splits as $i => $expense_split)
			  	<div class="section">
					<div class="col-md-12">
						<div class="panel panel-default">
...
{{-- 	<label for="amount" class="col-sm-4 control-label">Total</label> --}}
	<div class="col-sm-12">
		...
		<button type="submit" class="btn btn-success btn-block">Update Splits</button>
	</div>	
</div>
</div>
</div>
